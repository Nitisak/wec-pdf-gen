...
export async function getTermsTemplate(productVersion: string, language: string = 'en-US') {
  ...
  const template = await db
    ...
    .where(
      and(
        eq(htmlTemplate.kind, 'terms'),
        eq(htmlTemplate.productVersion, productVersion),
        eq(htmlTemplate.language, language)
      )
    )
...
export async function getDisclosureTemplate(stateCode: string, language: string = 'en-US') {
  const db = getDatabase();
  ...
  const template = await db
    ...
    .where(
      and(
        eq(htmlTemplate.kind, 'disclosure'),
        eq(htmlTemplate.stateCode, stateCode),
        eq(htmlTemplate.language, language)
      )
    )
    .orderBy(desc(htmlTemplate.createdAt))
...
export async function renderTermsToPdf(productVersion: string, policyNumber: string, language: string = 'en-US'): Promise<Uint8Array> {
...
export async function renderDisclosureToPdf(stateCode: string, language: string = 'en-US'): Promise<Uint8Array> {
